...
export function getClientToken(client: string): string {
	const hashed = hash(
		JSON.stringify({
			client,
			issuer: "jwtify",
	...
	return `{ "client": "${client}", "token": "${hashed}" }`;
}

export async function getAccessToken(clientToken: string, hourDuration: number = 1): Promise<string> {
	...
	return Promise.resolve("Bearer " + jwt.compact());
}
